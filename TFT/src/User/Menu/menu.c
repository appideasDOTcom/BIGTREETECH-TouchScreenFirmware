...
void selectmode(int8_t  nowMode)
{
  ...
  GUI_DispStringInRect(text_startx,rect_of_mode[1].y0-BYTE_HEIGHT+selecticonw+BYTE_WIDTH,LCD_WIDTH,rect_of_mode[1].y0+selecticonw+BYTE_WIDTH,(uint8_t *)"ostmoxy touch", 0);
  GUI_SetColor(FK_COLOR);
  GUI_DispStringInRect(0,rect_of_mode[1].y0-BYTE_HEIGHT+selecticonw+BYTE_WIDTH,text_startx,rect_of_mode[1].y0+selecticonw+BYTE_WIDTH,(uint8_t *)"ostmoxy complete", 0);
  }
  else
  {
  GUI_SetColor(ST7920_FNCOLOR);
